...
public class TabtwoFragment extends Fragment implements OnClickListener{
	
	protected static final String TAG = "PublishActivity";
	private String title = "爱情宣言";
	private int sendType = IConstants.SEND_TO_ALL;
	private View root;
	@Override
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
			Bundle savedInstanceState) {
		root = inflater.inflate(R.layout.tab_two, null);
		root.findViewById(R.id.device_btn).setOnClickListener(this);
		...
		if (TextUtils.isEmpty(content)) {
			...
			new AsyncTask<Void, Void, Boolean>(){
				...
				protected Boolean doInBackground(Void... params) {
					...
					boolean flag = new MPush(getActivity()).send(title, subHead,content,sendType);
					if (flag) {
